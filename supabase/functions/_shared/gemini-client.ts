...
export async function callGemini(request: GeminiRequest): Promise<GeminiResponse> {
  ...
  try {
    ...
    // Check for MAX_TOKENS finish reason - use partial content with warning
    if (candidate.finishReason === 'MAX_TOKENS') {
      console.warn('[Gemini] Response truncated due to MAX_TOKENS limit, using partial content');
      ...
      return {
        success: false,
        error: 'Response too long and no partial content available',
        model
      };
    }
