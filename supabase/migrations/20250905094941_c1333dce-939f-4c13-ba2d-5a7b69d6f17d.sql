...
ALTER TABLE public.startups 
ADD CONSTRAINT IF NOT EXISTS check_business_model CHECK (
  business_model IS NULL OR 
  business_model IN ('B2B', 'B2C', 'B2B2C', 'Marketplace', 'SaaS', 'Hardware', 'Services', 'Platform')
);

ALTER TABLE public.startups 
ADD CONSTRAINT IF NOT EXISTS check_investment_amount CHECK (
  total_investment_received IS NULL OR total_investment_received >= 0
);

-- Update existing linkedin_url constraint to be more flexible
ALTER TABLE public.startups 
DROP CONSTRAINT IF EXISTS check_linkedin_url;
