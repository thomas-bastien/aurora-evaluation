...
const EvaluationDashboard = () => {
  ...
  const stats = {
    ...
    averageScore: assignedStartups
      .filter(s => s.overall_score)
      .reduce((sum, s) => sum + (s.overall_score || 0), 0) / 
      assignedStartups.filter(s => s.overall_score).length || 0
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground mb-2">Startup Evaluations</h1>
