...
export const ResultsCommunication = ({
  ...
  const [selectedStartupForEmail, setSelectedStartupForEmail] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const [selectedStartupForVCFeedback, setSelectedStartupForVCFeedback] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const {
    data: vcFeedbackStatus,
    refetch: refetchVCFeedback
  } = useQuery({
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('startup_vc_feedback_details').select('startup_id, is_approved, evaluation_count').eq('round_name', currentRound === 'screeningRound' ? 'screening' : 'pitching');
  ...
  const loadEmailStatuses = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('startup_custom_emails').select('startup_id, is_approved').eq('round_name', currentRound === 'screeningRound' ? 'screening' : 'pitching').eq('communication_type', 'top-100-feedback');
      if (error) throw error;
  ...
  const handleViewVCFeedback = (startupId: string, startupName: string) => {
    setSelectedStartupForVCFeedback({
      id: startupId,
      name: startupName
    });
  ...
  const handleGenerateVCFeedback = async (startupId: string, startupName: string) => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('generate-vc-feedback-details', {
        body: {
          startupId,
    ...
    try {
      ...
      if (currentRound === 'pitchingRound') {
        ...
        const {
          ...
          `).eq('startup_round_statuses.rounds.name', 'screening').eq('startup_round_statuses.status', 'selected');
        if (error) throw error;
        startupsData = data;
      } else {
      ...
      const {
        data: roundStatusesData,
        error: roundStatusError
      } = await supabase.from('startup_round_statuses').select(`
          startup_id,
          status,
          rounds!inner(name)
      ...
      const {
        data: vcFeedbackData,
        error: vcFeedbackError
      } = await supabase.from('startup_vc_feedback_details').select('startup_id, plain_text_feedback, is_approved').eq('round_name', currentRound === 'screeningRound' ? 'screening' : 'pitching');
      if (vcFeedbackError) {
      ...
      } = await supabase.from('email_communications').select('recipient_id, status, sent_at').eq('recipient_type', 'startup').eq('round_name', currentRound === 'screeningRound' ? 'screening' : 'pitching').eq('communication_type', 'top-100-feedback').in('status', ['sent', 'delivered', 'opened', 'clicked']);
      if (sentEmailsError) {
  ...
  const generateFeedbackSummary = (name: string, strengths: string[], improvements: string[], notes: string[], score: number): string => {
...
The Aurora Evaluation Team`;
  ...
  const generateAIFeedback = async (startupId: string, startupName: string) => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('generate-founder-feedback', {
        body: {
          startupId,
          roundName: currentRound === 'screeningRound' ? 'screening' : 'pitching'
        }
      ...
      setStartupResults(prev => prev.map(result => result.id === startupId ? {
        ...result,
        feedbackSummary: formattedFeedback,
        feedbackStatus: 'draft'
      } : result));
  ...
  const loadTemplates = () => {
    const defaultTemplates: CommunicationTemplate[] = [{
      id: '1',
      type: 'selected',
      subject: currentRound === 'screeningRound' ? '🎉 Congratulations! You\'ve been selected for the Pitching Round' : '🎉 Congratulations! You\'ve been selected as a Finalist',
      content: currentRound === 'screeningRound' ? `Congratulations! Your startup has been selected to advance to the Pitching Round of our evaluation process.
...
Best regards,
The Aurora Team` : `Congratulations! Your startup has been selected as a Finalist in our evaluation process.
...
The Aurora Tech Awards Team`
  ...
  const handleSaveFeedback = async () => {
    ...
    setStartupResults(prev => prev.map(result => result.id === selectedResult.id ? {
      ...result,
      feedbackSummary: editingFeedback,
  ...
  const handleApproveFeedback = async (resultId: string) => {
    setStartupResults(prev => prev.map(result => result.id === resultId ? {
      ...result,
      feedbackStatus: 'approved'
    } : result));
  ...
  const generateAllFeedback = async (type: 'top-100-feedback') => {
    const startupsNeedingFeedback = selectedStartups.filter(s => s.feedbackSummary.includes('[AI Feedback not yet generated'));
    ...
    for (const startup of startupsNeedingFeedback) {
      try {
        ...
        if (!error && data.success && data.feedback) {
          ...
          setStartupResults(prev => prev.map(r => r.id === startup.id ? {
            ...r,
            feedbackSummary: formattedFeedback,
            feedbackStatus: 'draft'
          } : r));
  ...
  const batchApproveFeedback = (type: 'top-100-feedback') => {
    const startupsReadyForApproval = selectedStartups.filter(s => !s.feedbackSummary.includes('[AI Feedback not yet generated') && (s.feedbackStatus === 'draft' || s.feedbackStatus === 'reviewed'));
  ...
  const confirmBatchApproval = () => {
    ...
    setStartupResults(prev => prev.map(result => pendingBatchApproval.startupIds.includes(result.id) ? {
      ...result,
      feedbackStatus: 'approved'
    } : result));
  ...
  const enhanceSingleFeedback = async (startupId: string, startupName: string) => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('enhance-feedback-summary', {
        body: {
  ...
  const enhanceAllFeedback = async (type: 'top-100-feedback') => {
    const startupsToEnhance = selectedStartups.filter(s => !s.feedbackSummary.includes('[AI Feedback not yet generated') && s.feedbackStatus !== 'sent');
    ...
    for (const startup of startupsToEnhance) {
      try {
        ...
        if (!error && data.success && data.enhancedFeedback) {
          setStartupResults(prev => prev.map(r => r.id === startup.id ? {
            ...r,
            feedbackSummary: data.enhancedFeedback,
            feedbackStatus: 'reviewed'
          } : r));
          successCount++;
        }
  ...
  const applyEnhancement = () => {
    ...
    setStartupResults(prev => prev.map(result => result.id === enhancedFeedbackPreview.startupId ? {
      ...result,
      feedbackSummary: enhancedFeedbackPreview.enhanced,
      feedbackStatus: 'reviewed'
    } : result));
  ...
  const initiateCommunications = async (
    type: 'selected' | 'rejected' | 'under-review' | 'top-100-feedback' | 'approved',
    preFilteredResults?: StartupResult[]
  ) => {
    ...
    try {
      ...
      if (type === 'approved') {
        // Use pre-calculated approved startups (same as stat card logic)
        targetResults = preFilteredResults || approvedVCFeedback;
        console.log(`Approved communication: Using pre-calculated list with ${targetResults.length} startups`);
      ...
      const missingCount = targetResults.filter(s => s.feedbackSummary.includes('[AI Feedback not yet generated')).length;
      const unapprovedCount = targetResults.filter(s => !s.feedbackSummary.includes('[AI Feedback not yet generated') && s.feedbackStatus !== 'approved').length;
      if (missingCount > 0 || unapprovedCount > 0) {
        toast.error(`Cannot send emails: ${missingCount} missing feedback, ${unapprovedCount} unapproved`, {
  ...
  const sendCommunications = async () => {
    ...
    try {
      ...
      for (const validatedStartup of validStartups) {
        ...
        try {
          const {
            data,
            error
          } = await supabase.functions.invoke('send-individual-result', {
            body: {
              startupId: startup.id,
              startupName: startup.name,
              ...
              ...(pendingCommunicationType !== 'top-100-feedback' && {
                feedbackSummary: startup.feedbackSummary
              })
            }
          });
      ...
      if (successCount > 0) {
        toast.success(`Successfully sent ${successCount} emails` + (skippedCount > 0 ? `. ${skippedCount} startups were skipped due to validation issues.` : ''));
      }
  ...
  const sendIndividualEmail = async (result: StartupResult, communicationType: 'selected' | 'rejected' | 'under-review' | 'top-100-feedback') => {
    try {
      ...
      setStartupResults(prev => prev.map(r => r.id === result.id ? {
        ...r,
        feedbackStatus: 'sent',
        communicationSent: true
      } : r));
  ...
  const initiateRoundTransition = async () => {
    try {
      ...
      const {
        data: jurors,
        error: jurorError
      } = await supabase.from('jurors').select('*').not('user_id', 'is', null);
      if (jurorError) {
      ...
      const jurorNotifications = await Promise.allSettled(jurors.map(async juror => {
        const {
          count
        } = await supabase.from('screening_evaluations').select('*', {
          count: 'exact',
          head: true
        }).eq('evaluator_id', juror.user_id).eq('status', 'submitted');
      ...
      toast.success("Screening round completed! You can now switch to Pitching round to assign jurors to the selected startups.", {
        duration: 10000
      });
    } catch (error) {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Dialog open={showSendDialog} onOpenChange={setShowSendDialog}>
            ...
            <DialogContent className="max-w-md">
              ...
              <div className="space-y-4">
                <Button className="w-full" onClick={() => initiateCommunications('approved', approvedVCFeedback)} disabled={sendingEmails || validatingEmails || approvedVCFeedback.length === 0}>
                ...
                <Button className="w-full" variant="secondary" onClick={() => initiateCommunications('under-review')} disabled={sendingEmails || validatingEmails}>
                ...
                {currentRound === 'screeningRound' && <div className="pt-4 mt-4 border-t border-border">
        ...
        <Tabs defaultValue="feedback" className="space-y-6">
                ...
                return <div key={result.id} className="border border-border rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge variant={vcFeedbackStatusText === 'Approved' ? 'default' : vcFeedbackStatusText === 'Draft' ? 'secondary' : 'outline'}>
                      ...
                      {vcFeedbackStatusText === 'Not Generated' ? <Button variant="default" size="sm" onClick={() => handleGenerateVCFeedback(result.id, result.name)} disabled={generatingFeedback === result.id}>
                          ...
                          <Button size="sm" variant="outline" onClick={() => handleViewVCFeedback(result.id, result.name)}>
                            <Eye className="w-4 h-4 mr-2" />
                            VC Feedback
                          </Button>
                          <Button size="sm" variant="outline" onClick={() => handlePreviewEmail(result.id, result.name)}>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div className="flex items-center gap-2">
                        <div className="flex flex-col gap-1 items-end">
                          <Badge variant={emailStatus === 'approved' ? 'default' : emailStatus === 'draft' ? 'secondary' : 'outline'} className="text-xs">
                            Email: {emailStatus === 'approved' ? <>
                                <CheckCircle className="h-3 w-3 mr-1" />
                                Approved
                              </> : emailStatus === 'draft' ? 'Draft' : 'Not Generated'}
                          </Badge>
                          <Badge variant={vcStatus?.is_approved ? 'default' : 'outline'} className="text-xs">
                            VC: {vcFeedbackStatusText}
                          </Badge>
                        </div>
                        {result.communicationSent && <Badge className="bg-accent text-accent-foreground">Email Sent</Badge>}
                      </div>
          ...
          <TabsContent value="under-review" className="space-y-4">
            {underReviewStartups.map(result => <div key={result.id} className="border border-border rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-semibold text-foreground">{result.name}</h4>
                    <p className="text-sm text-muted-foreground">
                  ...
                  <div className="flex items-center gap-2">
                    {result.communicationSent ? <Badge className="bg-accent text-accent-foreground">Email Sent</Badge> : <Badge variant="outline">Pending</Badge>}
          ...
          <TabsContent value="rejected" className="space-y-4">
            {notSelectedStartups.map(result => <div key={result.id} className="border border-border rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-semibold text-foreground">{result.name}</h4>
                    <p className="text-sm text-muted-foreground">
        ...
        {/* Email Preview Modal */}
        {selectedStartupForEmail && <EmailPreviewModal open={showEmailPreview} onOpenChange={setShowEmailPreview} startupId={selectedStartupForEmail.id} startupName={selectedStartupForEmail.name} roundName={currentRound === 'screeningRound' ? 'screening' : 'pitching'} communicationType="top-100-feedback" onEmailStatusChange={handleEmailStatusChange} />}
        ...
        <Dialog open={showFeedbackDialog} onOpenChange={setShowFeedbackDialog}>
          <DialogContent className="max-w-2xl">
            ...
            <div className="space-y-4">
              <Textarea value={editingFeedback} onChange={e => setEditingFeedback(e.target.value)} rows={12} className="font-mono text-sm" />
            </div>
            <DialogFooter>
        ...
        <Dialog open={showBatchApprovalDialog} onOpenChange={setShowBatchApprovalDialog}>
          <DialogContent>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowBatchApprovalDialog(false)}>
                Cancel
              </Button>
              <Button onClick={confirmBatchApproval} disabled={batchApproving}>
                {batchApproving ? <>
                    <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                    Approving...
                  </> : <>
        ...
        <Dialog open={showEnhancementPreview} onOpenChange={setShowEnhancementPreview}>
          <DialogContent className="max-w-6xl max-h-[85vh] overflow-y-auto">
            ...
            {enhancedFeedbackPreview && <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Textarea value={enhancedFeedbackPreview.original} readOnly rows={15} className="font-mono text-xs bg-muted/50" />
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea value={enhancedFeedbackPreview.enhanced} readOnly rows={15} className="font-mono text-xs bg-primary/5 border-primary/20" />
                    <p className="text-xs text-muted-foreground">
                ...
                {enhancedFeedbackPreview.improvements.length > 0 && <Card className="bg-blue-50 border-blue-200">
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <ul className="text-sm text-blue-800 space-y-1">
                            {enhancedFeedbackPreview.improvements.map((improvement, idx) => <li key={idx} className="flex items-start gap-2">
        ...
        <StartupCommunicationConfirmationModal open={showConfirmationModal} onOpenChange={setShowConfirmationModal} communicationType={pendingCommunicationType} currentRound={currentRound} validationResults={validationResult?.validationResults || []} validationSummary={validationResult?.validationSummary || {
        totalStartups: 0,
        validStartups: 0,
        willSend: 0,
        willSkip: 0,
        skipReasons: {}
      }} onConfirm={sendCommunications} isLoading={sendingEmails} />

        {/* Email Preview Modal */}
        {showEmailPreview && selectedStartupForEmail && <EmailPreviewModal open={showEmailPreview} onOpenChange={setShowEmailPreview} startupId={selectedStartupForEmail.id} startupName={selectedStartupForEmail.name} roundName={currentRound === 'screeningRound' ? 'screening' : 'pitching'} communicationType="top-100-feedback" onEmailStatusChange={handleEmailStatusChange} refreshSignal={vcFeedbackRefreshSignal} />}

        {/* VC Feedback Details Modal */}
        {vcFeedbackOpen && selectedStartupForVCFeedback && <VCFeedbackDetailsModal open={vcFeedbackOpen} onOpenChange={setVcFeedbackOpen} startupId={selectedStartupForVCFeedback.id} startupName={selectedStartupForVCFeedback.name} roundName={currentRound === 'screeningRound' ? 'screening' : 'pitching'} onStatusChange={handleVCFeedbackStatusChange} />}
      </CardContent>
    </Card>;
};