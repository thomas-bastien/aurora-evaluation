...
export const MatchmakingWorkflow = ({
  currentRound
}: MatchmakingWorkflowProps) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const {
        data: assignmentStartupIds,
        error: assignmentIdsError
      } = await supabase.from(assignmentTable).select('startup_id').not('startup_id', 'is', null);
      ...
      const {
        ...
        error: roundStatusError
          ...
          status,
          rounds!inner(name)
        `).eq('rounds.name', currentRoundName).in('startup_id', startupsData?.map(s => s.id) || []);
      ...
      const {
        data: jurorsData,
        error: jurorsError
      } = await supabase.from('jurors').select('id, name, email, company, job_title, calendly_link, preferred_regions, target_verticals, preferred_stages, evaluation_limit, thesis_keywords, fund_focus, user_id').order('name');
      ...
      const filteredAssignments = mappedAssignments.filter(a => visibleStartupIds.has(a.startup_id));
  ...
  const fetchCommunicationStats = async () => {
    try {
      ...
      const {
        data: communications,
        error
      } = await supabase.from('email_communications').select('status, opened_at, clicked_at, template_id').eq('recipient_type', 'juror');
      ...
      const stats = {
        ...
        engagementRate: communications?.length > 0 ? communications.filter(c => c.opened_at).length / communications.length * 100 : 0
      };
  ...
  const getStartupAssignmentCount = (startupId: string): number => {
    return assignments.filter(assignment => assignment.startup_id === startupId && assignment.status !== 'cancelled').length;
  };
  ...
  const handleAssignmentComplete = (newAssignments: Assignment[]) => {
    setAssignments(prev => [...prev.filter(a => a.startup_id !== selectedStartup?.id), ...newAssignments]);
  ...
  const handleGenerateExplainableSuggestions = async () => {
    ...
    try {
      ...
      const result = await generateExplainableSuggestions(startupData, jurorData, roundName);
  ...
  const handleBulkAssign = async (newAssignments: {
    juror_id: string;
    startup_id: string;
  }[]) => {
    try {
  ...
  const handleConfirmAssignments = async () => {
    try {
      ...
      const {
        data: existing,
        error: existingError
      } = await supabase.from(assignmentTable).select('*').in('startup_id', startupIds);
      ...
      const deduplicatedExisting = Array.from((existing || []).reduce((map, a) => {
        const key = `${a.startup_id}-${a.juror_id}`;
        ...
        return map;
      }, new Map<string, any>())).map(([_, assignment]) => assignment);
      ...
      if (currentRound === 'pitchingRound') {
        ...
        } = await supabase.from('startup_round_statuses').select('startup_id, status, rounds!inner(name)').eq('rounds.name', 'screening').eq('status', 'selected').in('startup_id', startupIds);
        ...
        const assignmentsToValidate = assignments.filter(a => visibleStartupIds.has(a.startup_id));
      ...
      if (toDeleteIds.length > 0) {
        const {
          error: deleteError
        } = await supabase.from(assignmentTable).delete().in('id', toDeleteIds);
        if (deleteError) throw deleteError;
      }
      if (toInsert.length > 0) {
        const {
          error: insertError
        } = await supabase.from(assignmentTable).insert(toInsert);
  ...
  const handleSendSchedulingEmails = (filters: {
    onlyConfirmed: boolean;
    excludeAlreadyEmailed: boolean;
  ...
  const sendPitchSchedulingEmails = async (filters?: {
    onlyConfirmed: boolean;
    excludeAlreadyEmailed: boolean;
    forceOverride?: boolean;
  }) => {
    try {
      ...
      if (filters?.excludeAlreadyEmailed && !filters?.forceOverride) {
        const {
          data: emailedStartups
        } = await supabase.from('email_communications').select('recipient_id').eq('recipient_type', 'startup').or('subject.ilike.%scheduling%,subject.ilike.%pitch%').eq('status', 'sent');
      ...
      for (const startupId of eligibleStartupIds) {
        ...
        try {
          ...
          const assignedJurors = startupAssignmentList.map(assignment => {
          ...
          const {
            data,
            error
          } = await supabase.functions.invoke('send-pitch-scheduling', {
            body: {
  ...
  const sendAssignmentNotifications = async (filters?: {
      ...
      if (filters?.excludeAlreadyNotified) {
        ...
        const {
          ...
          error: notificationError
            ...
            recipient_id,
            metadata
          `).eq('recipient_type', 'juror').eq('communication_type', 'assignment-notification').in('recipient_id', Array.from(jurorAssignments.keys()));
        ...
        for (const [jurorId, jurorData] of Array.from(jurorAssignments.entries())) {
          ...
          const hasNewAssignments = currentStartupNames.some((startupName: string) => !previouslyNotifiedStartups.has(startupName));
  ...
  const roundDescription = currentRound === 'screeningRound' ? 'Assign 3 jurors to each startup for initial evaluation' : 'Assign 3 jurors to selected startups from Screening Round';
  return <div className="space-y-6">
        ...
        <CardHeader>
          ...
          <CardDescription className="font-body">
            <div className="space-y-1">
              ...
              <p className="text-sm">
                {currentRound === 'screeningRound' ? 'Assign 3 jurors to each startup. Jurors will evaluate pitch decks and provide scores/feedback.' : 'Re-assign 3 jurors to each startup selected for Pitching Round. Jurors will join pitch calls and evaluate presentations.'}
              </p>
            </div>
          </CardDescription>
        </CardHeader>
        <CardContent>
          {!hasCompleteData && <div className="mb-6 p-4 bg-warning/10 border border-warning/20 rounded-lg shadow-soft">
          ...
          <div className="flex gap-4 mb-6">
            <Button onClick={handleGenerateExplainableSuggestions} variant="outline" disabled={generatingSuggestions || jurors.length === 0} className="flex items-center gap-2 transition-smooth">
              <Zap className="w-4 h-4" />
              {generatingSuggestions ? 'Generating...' : 'Generate Suggestions'}
            </Button>

            <Button onClick={handleViewSummary} variant="outline" disabled={assignments.length === 0} className="flex items-center gap-2 transition-smooth">
              <Eye className="w-4 h-4" />
              View Assignment Summary
            </Button>
            
            <Button onClick={handleConfirmAssignments} disabled={assignments.length === 0 || isConfirmed} variant={isConfirmed ? "secondary" : "default"} className="flex items-center gap-2 transition-smooth">
          ...
          }} disabled={!isConfirmed || sendingEmails || sendingNotifications} variant={isEmailsSent ? "secondary" : "default"} className="flex items-center gap-2">
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold">
              ...
              {currentRound === 'screeningRound' && <span className="text-sm font-normal text-muted-foreground ml-2">
                  (Including rejected startups - assignment only available for non-rejected)
                </span>}
              {currentRound === 'pitchingRound' && <span className="text-sm font-normal text-muted-foreground ml-2">
            ...
            return <Card key={startup.id} className={`transition-all ${startup.roundStatus === 'rejected' ? 'bg-destructive/5 border-destructive/20' : isFullyAssigned ? 'bg-success/5 border-success/20' : ''}`}>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 space-y-2">
                        <div className="flex items-center gap-3">
                          ...
                          {assignmentCount === 0 ? <Badge variant="destructive" className="flex items-center gap-1 shrink-0">
                              <AlertCircle className="w-3 h-3" />
                              Unassigned
                            </Badge> : assignmentCount < requiredAssignments ? <Badge variant="outline" className="bg-warning/10 text-warning border-warning/20 flex items-center gap-1 shrink-0">
                              <AlertCircle className="w-3 h-3" />
                              ⚠️ {assignmentCount}/3 jurors
                            </Badge> : <Badge variant="default" className="flex items-center gap-1 shrink-0">
                      ...
                      return <Button onClick={() => handleAssignStartup(startup)} variant={isFullyAssigned ? "outline" : "default"} size="sm">
          ...
          {startups.length === 0 && <div className="text-center py-8">
      ...
      {/* Assignment Modal */}
      {selectedStartup && <StartupAssignmentModal startup={selectedStartup} jurors={jurors} existingAssignments={assignments.filter(a => a.startup_id === selectedStartup.id)} currentRound={currentRound} open={showAssignmentModal} onOpenChange={setShowAssignmentModal} onComplete={handleAssignmentComplete} />}
      ...
      <SendSchedulingEmailsModal open={showSchedulingEmailModal} onClose={() => setShowSchedulingEmailModal(false)} currentRound={currentRound} assignments={assignments} onSendEmails={handleSendSchedulingEmails} />

      {/* Assignment Notification Modal */}
      <AssignmentNotificationModal open={showAssignmentNotificationModal} onClose={() => setShowAssignmentNotificationModal(false)} currentRound={currentRound} assignments={assignments} onSendNotifications={sendAssignmentNotifications} />

      {/* Enhanced Auto-Assignment Panel */}
      {enhancedSuggestions && <EnhancedAutoAssignmentPanel suggestions={enhancedSuggestions.suggestions} whyNotAssigned={enhancedSuggestions.whyNotAssigned} allJurors={jurors.map(j => ({
      id: j.id,
      name: j.name,
      target_verticals: j.target_verticals || [],
      preferred_stages: j.preferred_stages || [],
    ...
    }))} open={showEnhancedAutoPanel} onOpenChange={setShowEnhancedAutoPanel} onApprove={handleBulkAssign} onCancel={() => {
      setShowEnhancedAutoPanel(false);
      setEnhancedSuggestions(null);
    }} config={{
